      ...
      >
        {navItems.map((navItem: any, idx: number) => (
          <Link
            ...
            href={navItem.link}
            className={cn(
              "relative link-underline dark:text-neutral-50 items-center flex space-x-1 text-neutral-600 dark:hover:text-neutral-300 hover:text-neutral-500"
            )}
          >
            {/* <span className="block sm:hidden">{navItem.icon}</span> */}
            <span className="text-sm shadow-md filter drop-shadow-md">{navItem.name}</span>
          </Link>
        ))}
      </motion.div>
    </AnimatePresence>
